...
(function(root,factory){
  ...
  else root.CustomerHomeView=api;
  ...
  function render(M,data,sort='work',mode='preview'){
    ...
    const groupHelp='차량 수가 다른 그룹을 비교할 수 있도록 대당 작업·대기시간을 같은 길이 척도로 표시합니다. 숫자는 대당 작업시간입니다. 대기비율순은 대기 ÷ (작업 + 대기)가 높은 순이며 비율을 산출할 수 없는 그룹은 마지막에 놓습니다. 배치·근무·기종 차이가 있으므로 성과 순위로 단정하지 않습니다. 그룹명을 누르면 금일 리포트, 에러·소모품은 해당 그룹의 서비스 목록으로 이동합니다.';
    return `<div class="od-content">
      ...
      ${staff?'':`<section class="dashboard-panel od-groups">${heading('그룹별 운영 비교','od-groups-help')}${groupSort}<div class="od-chart-caption"><span>${sort==='idle'?'대기 비율 높은 순':'대당 작업시간 많은 순'}</span>${legend}</div><div data-owner-groups>${groupRows}</div>${note('od-groups-help',groupHelp)}</section>`}
  ...
  return {build,current,sortGroups,render};
});
