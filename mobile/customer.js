(() => {
  ...
  function periodCutoff() { const s=periodState();return `<p class="source-note period-cutoff">${esc((state.view==='services'?M.periodWindow(s.from,s.to):M.efficiencyWindow(s.from,s.to)).label)}</p>`; }
  ...
  function readState() {
    ...
    return {view:Object.hasOwn(labels,view) ? view : 'home',role:Object.hasOwn(M.ROLE_LABELS,p.get('role')) ? p.get('role') : 'customer_owner',notificationCategory:notificationCategories.some(([key])=>key===p.get('notificationCategory'))?p.get('notificationCategory'):'all',serviceEquipmentId:view==='home'?'':p.get('serviceEquipmentId')||'',
      group:view==='home'?'':p.get('group')||'',type:'',q:view==='home'?'':p.get('q')||'',status:'all',live:view==='home'?'':p.get('live')||'',
  ...
  function go(view,patch={},replace=false) {
    ...
    if (view === 'home') Object.assign(next,{group:'',q:'',live:'',status:'all',equipmentId:'',reportVehicle:'',serviceEquipmentId:''});
  ...
  function periodControls({group=false,search=false,vehicle=false}={}) {
    ...
    return `<section class="compact-filter-panel ${hasGroup?'has-group':'no-group'} ${search?'has-search':'no-search'}${vehicle?' has-report-vehicle':''}" aria-label="조회 조건">
      ${hasGroup?'<div class="compact-filter-top">'+scopeControls():''}<div class="period-tabs" role="group" aria-label="조회 기간">${[['d',linked?'일':'금일'],['w','주'],['m','월']].map(([key,text])=>`<button type="button" data-period="${key}" aria-pressed="${s.period===key}" class="${s.period===key?'is-active':''}">${text}</button>`).join('')}</div>${hasGroup?'</div>':''}
      ${search?`<label class="vehicle-search compact-vehicle-search">${icon('search')}<input type="search" id="vehicle-search" value="${esc(state.q)}" placeholder="차량번호 · 기종 검색" aria-label="차량번호 · 기종 · 그룹 검색"></label>`:''}
      ${vehicle?reportVehicleControl():''}
  ...
  function reportActions(v) {
    return shortcutRow([
      `<button type="button" class="vehicle-compact-shortcut" data-vehicle-report="reports" data-equipment="${esc(v.equipmentId)}">${icon('chart-no-axes-combined')}<span>업무 현황</span>${icon('chevron-right')}</button>`,
      `<button type="button" class="vehicle-compact-shortcut" data-vehicle-report="efficiency" data-equipment="${esc(v.equipmentId)}">${icon('chart-no-axes-gantt')}<span>운영효율</span>${icon('chevron-right')}</button>`,
      `<button type="button" class="vehicle-compact-shortcut" data-route="shock">${icon('zap')}<span>충격 상세</span>${icon('chevron-right')}</button>`
  ...
  function vehicleActions(v,{energy=true,shock=true,maintenance=false}={}) {
    ...
    if(energy){
      ...
      links.push(`<button type="button" class="vehicle-compact-shortcut" data-slot="battery" ${applicable?`data-service-vehicle="${esc(v.equipmentId)}" data-kind="battery"`:'disabled'} aria-label="${label}" title="${label}">${icon('battery')}<strong>${value}</strong></button>`);
    }
  ...
  function home() {
    ...
    return `<div data-screen-id="LQ-DASH-001"><div class="snapshot"><h1>금일 현황</h1><span title="1시간 단위 수집 · 마지막 수집 완료 구간 기준 · 한국시간">${M.hourlyWindow().label}</span></div>
      ...
      <section class="dashboard-kpis health-kpis" aria-label="점검 필요 차량">${['error','due','soon'].map(key=>{const {label:title,icon:ico,tone}=issueStyle(key),cls='is-'+tone;return `<button class="dashboard-kpi ${cls}" type="button" data-service-focus="${key}"><span class="dashboard-kpi__label">${icon(ico)}<span>${title}</span></span><strong>${c[key]}<span class="kpi-denominator"> / ${c.total}대</span></strong><small>전체 차량 기준</small></button>`;}).join('')}</section>
  ...
  function reportVehicleControl() {
    const options=M.scope(rows,state);
    const valid=options.some(v=>v.equipmentId===state.reportVehicle);
    return `<label class="compact-report-vehicle"><select data-control="reportVehicle" aria-label="조회 차량"><option value="" ${!state.reportVehicle?'selected':''}>전체 차량</option>${state.reportVehicle&&!valid?'<option value="'+esc(state.reportVehicle)+'" selected disabled>조회할 수 없는 차량</option>':''}${options.map(v=>`<option value="${esc(v.equipmentId)}" ${v.equipmentId===state.reportVehicle?'selected':''}>${esc(v.equipmentNumber)} · ${esc(v.model)}</option>`).join('')}</select></label>`;
  }
  ...
  function details() {
    ...
    return `<div data-screen-id="LQ-OPS-002">${vehicleHeader(v)}<section class="detail-card"><div class="dashboard-panel__head"><h2>현재 점검 상태</h2><span>${v.conn?'수신 '+v.receivedAt.slice(5):'마지막 수신 '+v.receivedAt.slice(5)}</span></div>${vehicleActions(v,{shock:false,energy:false})}</section>
      ${periodControls()}<section class="detail-card"><div class="dashboard-panel__head"><h2>조회 기간 운행</h2><span>${isToday()?'금일':'조회 기간'}</span></div>${definition([['작업시간',m?hours(m.work):'-'],['대기시간',m?hours(m.idle):'-'],['운행시간',m?hours(m.min):'-'],['운행거리',m?fmt(m.km,' km'):'-'],['운영효율',rate(m?.efficiency)],['충격',m?fmt(m.shock,'건'):'-']])}${reportActions(v)}</section>
      <section class="detail-card"><div class="dashboard-panel__head"><h2>${energy} 정보</h2><span>${v.conn?'현재 수신':'마지막 수신'}</span></div>${v.type==='엔진'?definition([['평균 연료소비량',fmt(v.fc,' L/h')],['기준',M.ENERGY_MONTH+' 월간']]):definition([['종류',v.type],['잔량',rate(v.soc)],['평균 전력소비량',fmt(v.bc,' kWh/h (월간)')],['배터리 전압',fmt(v.batteryVoltage,' V')],['배터리 용량',fmt(v.batteryCapacity,' Ah')],['수신시각',v.receivedAt]])}<p class="source-note">${energy==='엔진'?'회전수·냉각수 온도 이력 미수신':'시계열·충전 이력 미수신'}</p></section>
  ...
  function child() {
    ...
    if(kind==='battery') content=v.type!=='엔진'?definition([['배터리 종류',v.type],['잔량'+(!v.conn?' (마지막 수신)':''),v.soc!=null?v.soc+'%':'-'],['수신시각',v.receivedAt],['평균 전력소비량 ('+M.ENERGY_MONTH+')',v.bc!=null?v.bc+' kWh/h':'-'],['배터리 전압',fmt(v.batteryVoltage,' V')],['배터리 용량',fmt(v.batteryCapacity,' Ah')],['충전 예약','이용 불가']]):'<p class="empty-state">배터리 차량이 아닙니다.</p>';
    if(kind==='supplies') content=M.supplyItems(v).map(supplyDetail).join('')||'<p class="empty-state">등록된 소모품이 없습니다.</p>';
    ...
    if(state.serviceEntry&&['error','maintenance','supplies'].includes(kind)) {
    ...
    return `<div data-screen-id="${ids[kind]}">${vehicleHeader(v)}<div class="section-heading"><h2>${labels[kind]}</h2><small>${kind==='battery'&&!v.conn?'마지막 수신 정보':kind==='error'&&state.serviceEntry?'발생 이력':['battery','supplies','error'].includes(kind)?'현재 수신 상태':kind==='engine'?M.ENERGY_MONTH+' 월간':'기간별 정보'}</small></div>${['operation','shock'].includes(kind)?periodControls():''}<section class="detail-card">${content}</section>${dataNote('운행·충격은 선택 기간 기준, 연료·전력소비량은 '+M.ENERGY_MONTH+' 현재 집계 기준입니다. 잔량·점검 알림은 마지막 수신 기준이며 수신되지 않은 숫자는 -로 표시합니다.')}<button class="detail-secondary-button" type="button" data-return>${state.returnView==='notifications'?'알림 목록으로 돌아가기':state.returnView==='summary'?'요약정보로 돌아가기':state.returnView==='services'?'서비스 목록으로 돌아가기':'차량 상세로 돌아가기'}</button></div>`;
  }
  ...
  function supplyDetail(r) {
    return `<article class="supply-detail"><h3>${esc(r.name)}</h3>${supplyUsage(r)}${definition([['교체주기',r.cycleHours>0?fmt(r.cycleHours)+' h':'-'],['사용시간',r.usedHours!=null?fmt(r.usedHours)+' h':'-'],['최근 교체일',r.lastChangedAt||'미수신'],['수집 기준',r.occurredAt]])}</article>`;
  }
  function services() {
  ...
  function reports() {
    ...
    if(state.reportVehicle&&!vehicle) return `<div data-screen-id="LQ-RPT-001"><h1>업무 리포트</h1>${reportTabs()}${periodControls({group:true,vehicle:true})}${reportScopeError()}</div>`;
    ...
    return `<div data-screen-id="LQ-RPT-001"><div class="snapshot"><h1>업무 리포트</h1><span>${state.role==='customer_owner'?(state.group||'업체 전체'):'내 그룹'}</span></div>${reportTabs()}${periodControls({group:true,vehicle:true})}
  ...
  function efficiency() {
    ...
    if(state.reportVehicle&&!vehicle) return `<div data-screen-id="LQ-REF-003"><h1>운영효율</h1>${reportTabs()}${periodControls({group:true,vehicle:true})}${reportScopeError()}</div>`;
    ...
    return `<div data-screen-id="LQ-REF-003"><div class="snapshot"><h1>운영효율</h1><span>${state.role==='customer_owner'?(state.group||'업체 전체'):'내 그룹'}</span></div>${reportTabs()}${periodControls({group:true,vehicle:true})}<section class="dashboard-panel"><div class="dashboard-panel__head">${criteriaHeading('기간 운영효율','efficiency-help')}<span>집계 ${p.known} / ${p.total}대</span></div>${criteriaPanel('efficiency-help',referenceRateCriteria(today)+'<p class="source-note">'+efficiencyScopeCriteria+'</p>')}${reportVehicleLink(vehicle)}${!p.known?'<p class="empty-state">집계 정보가 없습니다.</p>':''}<div class="summary-totals"><span>운영효율<strong>${rate(p.workShare)}</strong></span><span>작업시간<strong>${shortHours(p.known?Math.round(p.work/p.known):null)}</strong></span><span>대기시간<strong>${shortHours(p.known?Math.round(p.idle/p.known):null)}</strong></span></div></section>
  ...
  function openLocationDialog() {
    ...
    $('#location-dialog-info').innerHTML=cell('기종',v.model)
      ...
      +cell('고객사',v.companyName,true)+cell('위치 기준 시간',v.receivedAt,true);
    $('#location-dialog-coordinate').textContent='('+p.lat.toFixed(6)+', '+p.lng.toFixed(6)+')';
    $('#location-map').src='https://maps.google.com/maps?q='+encodeURIComponent(p.lat+','+p.lng)+'&z=16&hl=ko&output=embed';
  ...
  document.addEventListener('close',event=>{
    if(event.target===$('#location-dialog')){setLocationMapExpanded(false);$('#location-map').removeAttribute('src');$('[data-map]')?.focus();}
  },true);
