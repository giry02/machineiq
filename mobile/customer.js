(() => {
  ...
  const socAttributes = v => v.type==='리튬'&&Number.isFinite(v.soc)&&v.soc>=0&&v.soc<=100 ? `class="battery-soc-value" data-soc-level="${v.soc>=60?'high':v.soc>=30?'medium':'low'}"` : '';
  const definition = entries => `<dl class="detail-definition">${entries.map(([k,v,attributes=''])=>`<div><dt>${esc(k)}</dt><dd${attributes?' '+attributes:''}>${esc(v)}</dd></div>`).join('')}</dl>`;
  ...
  function vehicleActions(v,{energy=true,shock=true,maintenance=false}={}) {
    ...
    if(energy){
      ...
      links.push(`<button type="button" class="vehicle-compact-shortcut" data-slot="battery" ${applicable?`data-service-vehicle="${esc(v.equipmentId)}" data-kind="battery"`:'disabled'} aria-label="${label}" title="${label}">${icon('battery')}<strong ${socAttributes(v)}>${value}</strong></button>`);
    }
  ...
  function details() {
    ...
    return `<div data-screen-id="LQ-OPS-002">${vehicleHeader(v)}<section class="detail-card"><div class="dashboard-panel__head"><h2>현재 점검 상태</h2><span>${v.conn?'수신 '+v.receivedAt.slice(5):'마지막 수신 '+v.receivedAt.slice(5)}</span></div>${vehicleActions(v,{shock:false,energy:false})}</section>
      ...
      <section class="detail-card"><div class="dashboard-panel__head"><h2>${energy} 정보</h2><span>${v.conn?'현재 수신':'마지막 수신'}</span></div>${v.type==='엔진'?definition([['평균 연료소비량',fmt(v.fc,' L/h')],['기준',M.ENERGY_MONTH+' 월간']]):definition([['종류',v.type],['잔량',rate(v.soc),socAttributes(v)],['평균 전력소비량',fmt(v.bc,' kWh/h (월간)')],['배터리 전압',fmt(v.batteryVoltage,' V')],['배터리 용량',fmt(v.batteryCapacity,' Ah')],['수신시각',v.receivedAt]])}<p class="source-note">${energy==='엔진'?'회전수·냉각수 온도 이력 미수신':'시계열·충전 이력 미수신'}</p></section>
  ...
  function child() {
    ...
    if(kind==='battery') content=v.type!=='엔진'?definition([['배터리 종류',v.type],['잔량'+(!v.conn?' (마지막 수신)':''),v.soc!=null?v.soc+'%':'-',socAttributes(v)],['수신시각',v.receivedAt],['평균 전력소비량 ('+M.ENERGY_MONTH+')',v.bc!=null?v.bc+' kWh/h':'-'],['배터리 전압',fmt(v.batteryVoltage,' V')],['배터리 용량',fmt(v.batteryCapacity,' Ah')],['충전 예약','이용 불가']]):'<p class="empty-state">배터리 차량이 아닙니다.</p>';
