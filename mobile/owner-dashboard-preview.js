...
(function(root,factory){
  ...
  function current(M,vehicles,through=M.SNAPSHOT){
    ...
    return {total:vehicles.length,known:entries.length,offline:vehicles.length-entries.length,running,idleCount:entries.length-running,
      ...
      due:supplies.filter(s=>s.key==='due').length,soon:supplies.filter(s=>s.key==='soon').length};
  }
  ...
  function render(M,data,sort='work',mode='preview'){
    ...
    return `<div class="od-content">
      ...
      <section class="dashboard-panel od-priority" aria-label="금일 에러 발생 건수와 현재 소모품 항목 수"><div class="od-issues">${issue('error','에러 발생','triangle-alert')}${issue('due','교체 필요','refresh-cw')}${issue('soon','교체 임박','clock')}</div></section>
      ...
      <section class="dashboard-panel od-groups">${heading(staff?'내 그룹 작업 현황':'그룹별 운영 비교','od-groups-help')}${groupSort}<div class="od-chart-caption"><span>${staff?'대당 작업·대기시간':sort==='idle'?'대기 비율 높은 순':'대당 작업시간 많은 순'}</span>${legend}</div><div data-owner-groups>${groupRows}</div>${note('od-groups-help',groupHelp)}</section>
      <section class="dashboard-panel od-trend">${heading('최근 7일 작업 추이','od-trend-help')}<div class="od-trend-summary"><div><span>일평균 작업시간</span><strong>${h(avg)}</strong></div><span>${trend[0].day.slice(5).replace('-','.')}~${trend[6].day.slice(5).replace('-','.')}<br>오늘 제외</span></div><div class="od-chart-caption"><span>운행시간 · h</span>${legend}</div><div class="od-trend-bars" aria-label="최근 7일 일별 작업·대기시간 차트">${bars}</div><p class="od-footnote">날짜를 누르면 해당 일자의 리포트가 열립니다.</p>${note('od-trend-help','어제까지 완료된 7개 날짜의 기간별 리포트 실적입니다. 숫자는 일별 운행시간(작업 + 대기)을 시간 단위 정수로 반올림하며 막대의 회색 부분은 대기시간입니다. 오늘은 집계 중이므로 추이에서 제외합니다. 현재 미연결 차량도 해당 과거 날짜에 수집된 실적이 있으면 포함하므로, 연결 차량만 집계한 오늘 카드와 조회 대상이 다릅니다. 미수신 날짜는 0이 아닌 -입니다.')}</section>
    </div>`;
  }
