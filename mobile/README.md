...
2026-09-23 r96. 이 폴더만으로 실행하는 고객 모바일 HTML 시연본입니다. 대표는 업체 전체, 직원은 배정 그룹을 조회합니다. 실제 인증·회원가입·PUSH·등록 서버는 연결하지 않았습니다.
...
HTML에는 QR·카메라·네이티브 권한 UI가 없습니다. 지도는 키 없이 Google 지도 외부 열기만 제공하며 인터넷이 필요합니다. 실제 충전 명령·이메일/SMS/PUSH 발송은 없습니다.
