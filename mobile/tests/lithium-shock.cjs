...
const ctx=vm.createContext({window:{},Date});vm.runInContext(fleetSource,ctx);vm.runInContext(fs.readFileSync(path.join(app,'model.js'),'utf8'),ctx);vm.runInContext(lithiumSource,ctx);
...
for(const v of rows){
  ...
  for(let d=1;d<=31;d++){const day='2026-08-'+String(d).padStart(2,'0'),m=M.metrics(v,day,day);for(const key of Object.keys(sum))sum[key]+=m?.shockBands?.[key]||0;}
  assert.equal(JSON.stringify(sum),JSON.stringify(month.shockBands),'Daily bands add up to monthly bands');
...
h.click({},'data-save-charge');assert.equal(store.size,1);assert(h.html().includes('저장했습니다. (시연)'));
